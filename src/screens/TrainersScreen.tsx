...
import type { User, TrainerConnection } from '../types';
...
export default function TrainersScreen({ 
    ...
    const activeConnections = useMemo(() => {
        const activeIds = connections.filter(c => c.status === 'active').map(c => c.trainer_id);
        return trainers.filter(t => activeIds.includes(t.id));
    ...
    return (
        <div className="max-w-md mx-auto w-full min-h-screen bg-gray-50 flex flex-col relative shadow-xl overflow-hidden">
                ...
                {selectedTrainerDetail && showGoalApproval && (
                    <TrainerGoalApprovalScreen
                        ...
                        onDone={() => {
                            ...
                            setActiveTab('my');
                            onViewProfile(selectedTrainerDetail);
                        }}
                    />
                )}
            </AnimatePresence>
            ...
            <nav className="absolute bottom-0 left-0 right-0 h-[56px] bg-white border-t-[0.5px] border-[#E5E7EB] flex items-center justify-around px-[10px] z-[50]">
                ...
                <NavButton label="Trainers" icon={Users} active={true} onClick={onFindTrainer} />
                <NavButton label="Progress" icon={BarChart3} onClick={onViewProgress} />
                <NavButton label="Alerts" icon={Bell} badgeContent={unreadAlertsCount} onClick={onViewAlerts} />
            </nav>
        </div>
    );
}
