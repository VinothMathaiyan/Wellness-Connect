...
export interface TrainerMessageThreadViewProps {
  /** The other participant in the thread. */
  otherUserId: string;
  /** The client being discussed (route/picker state); resolved from the thread
      when not provided. */
...
/**
 * Reusable trainer message-thread view (header + messages + composer).
 * Used by TrainerMessageThreadScreen (mobile push-navigation route) and inline
 * by TrainerMessagesScreen at lg: (master-detail). Data flow is identical in
 * both usages: getMessageThread + markMessagesRead on mount / otherUserId
 * change, sendAssessmentMessage (with the resolved clientId) on send.
 */
export default function TrainerMessageThreadView({
  otherUserId,
  ...
  useEffect(() => {
    ...
    async function fetchThread() {
      ...
      try {
        ...
        // Resolve the clientId being discussed: provided state → thread's
  ...
  }, [userId, otherUserId, initialClientId]);
